...
class WC_Gateway_Adjeminpay_IPN_Handler extends WC_Gateway_Adjeminpay_Response {
    ...
    protected function payment_status_completed( $order, $posted ) {
        ...
        if ( 'SUCCESSFUL' === $posted['status'] ) {
            ...
            $this->payment_complete( $order, ( ! empty( $posted['transaction_id'] ) ? wc_clean( $posted['transaction_id'] ) : '' ), __( 'IPN payment completed', 'adjeminpay' ) );
    ...
    protected function save_adjeminpay_meta_data( $order, $posted ) {
        ...
        if ( ! empty( $posted['transaction_id'] ) ) {
            update_post_meta( $order->get_id(), '_transaction_id', wc_clean( $posted['transaction_id'] ) );
        }
